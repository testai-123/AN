'''
    ...
    Explanation - 
        The problem we're trying to solve here is training a computer program to recognize the XOR (Exclusive OR) logic. XOR is a binary operation that gives a 
        true output only when the inputs are different (one is 0 and the other is 1). For any other combination (both 0s or both 1s), the output is false.
'''
